...
struct kcsbridged_context {
	...
	/*
	 * Request and Response Messages are paired together as a Write Transfer to
	 * the BMC to send the request followed by a Read Transfer from the BMC to
	 * get the response.
	 */
	int expired;
	uint8_t seqnum;
...
static void kcs_log(int p, const char *fmt, ...)
{
	va_list args;
...
static int handle_kcs_request(struct kcsbridged_context *context, uint8_t *msg, size_t msglen)
{
	struct kcs_msg_req *req;

	if (msglen < 2) {
		MSG_ERR("KCS message with a short length (%d)\n", msglen);
		return -1;
	}
...
static int method_send_message(sd_bus_message *msg, void *userdata, sd_bus_error *err)
{
...
static int method_set_sms_atn(sd_bus_message *msg, void *userdata, sd_bus_error *err)
{
	struct kcsbridged_context *context = userdata;
	int r;
...
static int method_clear_sms_atn(sd_bus_message *msg, void *userdata, sd_bus_error *err)
{
	struct kcsbridged_context *context = userdata;
	int r;
...
static int method_force_abort(sd_bus_message *msg, void *userdata, sd_bus_error *err)
{
	struct kcsbridged_context *context = userdata;
	int r;
...
static int dispatch_kcs(struct kcsbridged_context *context)
{
	...
	r = sd_bus_message_append(msg, "yyyy", context->seqnum, req->netfn, req->lun, req->cmd);
	if (r < 0) {
...
{
	fprintf(stderr, "Usage %s [--v[v] | --syslog] --d <DEVICE>\n"
			"--v                     Be verbose\n"
...
static const sd_bus_vtable ipmid_vtable[] = {
	...
	SD_BUS_METHOD("setAttention",   "",        "x", &method_set_sms_atn,   SD_BUS_VTABLE_UNPRIVILEGED),
	SD_BUS_METHOD("clearAttention", "",        "x", &method_clear_sms_atn, SD_BUS_VTABLE_UNPRIVILEGED),
	SD_BUS_METHOD("forceAbort",     "",        "x", &method_force_abort,   SD_BUS_VTABLE_UNPRIVILEGED),
