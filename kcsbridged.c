...
#define OPTMAXLEN (NAMEBUFFER - 23)
...
int main(int argc, char *argv[])
{
	...
	while ((opt = getopt_long(argc, argv, "", long_options, NULL)) != -1) {
		...
		case 'd':
			snprintf(kcsDevice, NAMEBUFFER, "%s", optarg);
			deviceOptFlag = true;
			break;
		...
		case 'i':
			...
			snprintf(busName, NAMEBUFFER, "%s%s", DBUS_NAME,
				 optarg);
			snprintf(objPath, NAMEBUFFER, "%s%s", OBJ_NAME, optarg);
			dbusOptFlag = true;
			break;
