                               ...
                               sd_bus_error *err)
{
    ...
    MSG_OUT("Send rsp msg <- seq=0x%02x netfn=0x%02x lun=0x%02x cmd=0x%02x "
            "cc=0x%02x\n",
...
static void usage(const char *name)
{
    fprintf(stderr,
            ...
            "--s, --syslog            Log output to syslog (pointless without "
            "--verbose)\n"
...
int main(int argc, char *argv[])
{
    ...
    context = static_cast<kcsbridged_context *>(calloc(1, sizeof(*context)));
    if (!context)
    {
        fprintf(stderr, "OOM!\n");
        return -1;
    ...
    while ((opt = getopt_long(argc, argv, "", long_options, NULL)) != -1)
    {
        switch (opt)
        {
            ...
            case 'i':
                ...
                snprintf(busName, NAMEBUFFERLEN, "%s%s", DBUS_NAME, optarg);
                snprintf(objPath, NAMEBUFFERLEN, "%s%s", OBJ_NAME, optarg);
                break;
