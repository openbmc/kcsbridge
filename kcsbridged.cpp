...
class SmsChannel
{
  public:
    ...
    void asyncWaitForIncoming()
    {
        dev->async_wait(boost::asio::posix::stream_descriptor::wait_read,
                        [this](const boost::system::error_code& ec) {
                            ...
                            {
                                channelAbort("Error while waiting for read",
                                             ec);
                            }
                        });
    }
    ...
    void processMessage()
    {
        boost::system::error_code ecRd;
        std::vector<uint8_t> raw(kcsMessageSize);
        size_t rlen =
            boost::asio::read(*dev, boost::asio::buffer(raw),
                              boost::asio::transfer_at_least(2), ecRd);
        if (ecRd || rlen < 2)
        {
        ...
        // copy out payload
        std::vector<uint8_t> data(rawIter, raw.end());
        ...
        bus->async_method_call(
            [this, netfnCap{netfn}, lunCap{lun}, cmdCap{cmd}](
                boost::system::error_code ec, const IpmiDbusRspType& response) {
