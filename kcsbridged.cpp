...
class SmsChannel
{
  public:
    ...
    void asyncReadNextMessage()
    {
        boost::asio::async_read(
            *dev,
            boost::asio::mutable_buffer(rawMessage.data(), rawMessage.size()),
            boost::asio::transfer_at_least(2),
            [this](const boost::system::error_code& ec, size_t bytesRead) {
                processMessage(ec, bytesRead);
            });
    }

    void processMessage(const boost::system::error_code& ec, size_t bytesRead)
    {
        if (ec || bytesRead < 2)
        {
            channelAbort("Failed to read req msg", ec);
            return;
        }
        ...
        auto rawIter = rawMessage.begin();
        auto rawEnd = rawMessage.begin() + bytesRead;
  ...
  protected:
    std::array<uint8_t, kcsMessageSize> rawMessage;
